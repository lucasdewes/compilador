#    Chronos Analisador Lexico

# --- ERROS LEXICOS -----
# O simbolo nao pertencente ao conjunto de simbolos terminais da linguagem
...
# Declaracao da Classe do analisador Lexico
class AnalisadorLexico():
    ...
    def __init__(self):
        self.arquivo_e = nome_do_arquivo
        self.arquivo_s = "resultado-lex.chr"

    # Metodo que verifica se a entrada eh um delimitador
    #  find() retorna a posicao do caractere na string de entrada caso o mesmo seja encontrado
    ...
    def qualTokenOperador(self, entrada): # aqui detecta o < & >
        # Listas com os operadores componentes da linguagem
        operadores = '= + - * / < > ++ -- == != >= <= ||'.split()
        posicao = 0
        for x in operadores:
    ...
    def analisa(self):
        ...
        while linha_programa:
            ...
            while i < tamanho_linha:  # Percorre os caracteres da linha
                ...
                if self.Delimitador(caracter_atual):
                    arquivo_saida.write(
                        self.qualTokenDelimitador(caracter_atual) + ',' + caracter_atual + ',' + str(
                            numero_linha) + ',' + str(i) + '\n')
                # ===================================================================================
                ...
                elif caractere_seguinte is not None and self.Operador(caracter_atual + caractere_seguinte):
                    arquivo_saida.write(self.qualTokenOperador(
                        caracter_atual + caractere_seguinte) + ',' + caracter_atual + caractere_seguinte + ',' + str(
                        numero_linha) + ',' + str(i) + '\n')
                    i += 1
                elif self.Operador(caracter_atual):
                    arquivo_saida.write(
                        self.qualTokenOperador(caracter_atual) + ',' + caracter_atual + ',' + str(
                            numero_linha) + ',' + str(i) + '\n')
                ...
                # string.punctuation[6] retorna o simbolo - ' - que representa o inicio do caractere constante
                    ...
                    if (linha_programa[i + 1] == '\n') or (not (string.punctuation[6] in linha_programa[i + 1:])):
                        ...
                        i = tamanho_linha
                    elif self.Simbolo(linha_programa[i + 1]) and linha_programa[i + 1] != string.punctuation[6] and \
                ...
                # Verificando se o elemento em questao eh um numero - OK
                    ...
                    if caracter_atual == '.':
                        if (i + 1) < tamanho_linha:
                            ...
                            if caracter_atual == '.':
                                ...
                                while i + 1 < tamanho_linha:
                                    i += 1
                                    caracter_atual = linha_programa[i]
                                    if self.Delimitador(caracter_atual) or caracter_atual == ' ':
                        ...
                        arquivo_saida.write('literalzahl,' + string_temp + ',' + str(
                            numero_linha) + ',' + str(i) + '\n')
                        if not self.Digito(caracter_atual):
                            i -= 1
                # ===================================================================================
                # Verificando identificadores ou palavras reservadas - OK
                elif self.Letra(caracter_atual):
                    ...
                    while i < tamanho_linha:
                        ...
                        if i + 1 < tamanho_linha:
                            caractere_seguinte = linha_programa[i + 1]
                        if self.Letra(caracter_atual) or self.Digito(caracter_atual) or caracter_atual == '_':
                            string_temp += caracter_atual
                            ...
                            break
                        elif (caractere_seguinte is not None and self.Operador(
                                caracter_atual + caractere_seguinte)) or self.Operador(caracter_atual):
                            i -= 1
                            break
                        elif caracter_atual != '\n':
                            arquivo_saida.write(
                    ...
                    if algum_erro:
                        ...
                        if self.Reservada(string_temp):
                            ...
                            arquivo_saida.write(
                                self.qualTokenReservada(string_temp) + ',' + string_temp + ',' + str(
                                    numero_linha) + ',' + str(i) + '\n')
                        else:
                            arquivo_saida.write(
